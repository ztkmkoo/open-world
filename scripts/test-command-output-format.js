...
async function main() {
  ...
  try {
    ...
    db.prepare(`
      INSERT INTO users (
        user_id, surname, given_name, nickname_unique, gender,
        sect_id, squad_id, role_layer_key, created_at, updated_at
      )
      VALUES (
        'utest_fmt', '문', '식', 'format_case_user', 'M',
        'SECT_CHEONGUN', 'CHEONGUN_SQUAD_CHEONGSAN_1', 'L1_MEMBER',
    ...
    assert(help.status === 200 && help.json.ok === true, "help command should succeed");
    assert(Array.isArray(help.json.lines) && help.json.lines.length > 0, "help command should return lines");

    const status = await postJson(`${baseUrl}/command`, { input: "상태" }, cookie);
    assert(status.status === 200 && status.json.ok === true, "status command should succeed");
    const statusLines = Array.isArray(status.json.lines) ? status.json.lines.join("\n") : "";
    assert(statusLines.includes("직위:"), "status output should include role line");
    assert(statusLines.includes("위치:"), "status output should include location line");

    const trainingList = await postJson(`${baseUrl}/command`, { input: "수련 목록" }, cookie);
    assert(trainingList.status === 200 && trainingList.json.ok === true, "training list command should succeed");
